...
const daysInMatrix = useCalendarMatrix(currentYear, currentMonth);
...
<template>
    <div ref="containerRef" :class="['flex flex-col gap-1 font-sans relative', containerClass]">
        <span v-if="label" :class="['text-base text-secondary-800 font-medium select-none w-fit mb-0.5', labelClass]">
            {{ label }}
        </span>
        ...
        <div v-if="activePicker" :class="[
            ...
            <div class="grid grid-cols-7 gap-y-1 text-center text-base">
                <div v-for="(cell, idx) in daysInMatrix" :key="idx"
                    class="flex items-center justify-center h-8 font-normal">
